...
import (
	...
	"github.com/bingoohuang/pb/termutil"
...
const (
	...
	// SIBytesPrefix Use SI bytes prefix names (kB, MB, etc) instead of IEC prefix names (KiB, MiB, etc)
	SIBytesPrefix
	...
	// CleanOnFinish Hide the progress bar when finished, rather than leaving it up. By default, it's false.
	CleanOnFinish

	// TimeRound elapsed time to this precision. Defaults to time.Second.
	TimeRound
)
...
const (
	...
	defaultNoTerminalRefreshRate = time.Second * 3
)

// New creates new ProgressBar object
func New(total int) *ProgressBar { return New64(int64(total)) }
...
func StartNew(total int) *ProgressBar { return New(total).Start() }

// Start64 starts new ProgressBar with Default template. Using int64 as total.
func Start64(total int64) *ProgressBar { return New64(total).Start() }
...
func (i *atomicInt64) LoadInt64() int64       { return atomic.LoadInt64(&i.v) }
func (i *atomicInt64) StoreInt64(value int64) { atomic.StoreInt64(&i.v, value) }
func (i *atomicInt64) AddInt64(value int64)   { atomic.AddInt64(&i.v, value) }

// ProgressBar is the main object of bar
type ProgressBar struct {
	current, total atomicInt64
	width          int
	maxWidth       int
	mu             sync.RWMutex
	rm             sync.Mutex
...
func (pb *ProgressBar) configure() {
	...
	if pb.refreshRate == 0 {
		if tm, ok := pb.vars[Terminal].(bool); ok && tm {
			...
			pb.refreshRate = defaultNoTerminalRefreshRate
		}
	}
...
// SetTemplate sets ProgressBarTempate and parse it
...
// String return current string representation of ProgressBar
